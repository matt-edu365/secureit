...
- The image seeds or refreshes `/var/www/data/canonical-controls.json` on startup if the live volume does not already contain it, or if the file is empty or resolves to zero controls, so the homepage should show the real M365 security checks total instead of zero.
